...
(2026-07-17 6차 완료: ⑥상세 시트 타임피커 — detail.js 시간 필드 4곳을 compose `fillTimePickerPopover` 재사용 버튼으로 교체 + 이벤트 상세 `t` 섀도잉 크래시 수정)

1. fullpage 레일에 드로어식 화이트 잔액 카드(강조 1곳) 이식 검토
2. 검색 시트 — 키보드 단축키(Cmd/Ctrl+K) 진입 검토
