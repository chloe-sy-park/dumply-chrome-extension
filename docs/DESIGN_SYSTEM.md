...
- **섹션 카드 = `.a-card` 단일 문법** (2026-07-17 덤프 탭 통일): 타이틀은 카드 *안* 헤드(메인 14/600 잉크 + 서브 11 보조), 헤드 `8px 14px`, 행 `12px 16px` + `--border` 구분선 + hover `accent-soft`, 푸터 = accent-soft 밴드 + 상단 구분선. 카드 밖 eyebrow 섹션 레이블 문법은 폐기(mind-section 잔존분 제외). 카드 안 구조 아이콘은 잉크(액센트 금지 — 시안의 `var(--accent)`는 액센트 축 이전 작성분).
