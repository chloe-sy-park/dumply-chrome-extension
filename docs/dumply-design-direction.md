...
- PR #29의 base(`4894342`)는 이후 main에 머지된 크레딧 시스템(#30)·랜딩 개편(#31)보다 오래됨 → 머지하기로 결정하면 **리베이스 필요**.
...
1. **PR #29 방향 결정** — v0.1 웜 듄 유지 vs v0.2 Zinc 채택. 채택 시 main 리베이스 + 확장 `tokens.css`와의 정합성 검토 필요.
2. 알프레도 정식 일러스트 반입 (5상태 + celebrating 요일 variant 7종).
