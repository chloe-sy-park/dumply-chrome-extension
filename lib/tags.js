...
const AlfredoTags = (() => {
  ...
  // 망설임/미결정 표지 (고민)
  const PONDER_RE = /(할까\s*말까|갈까\s*말까|살까\s*말까|할지\s*말지|할까|말까|해야\s*하나|해야\s*되나|해야\s*할까|어떡하|어떻게\s*하지|어떻게\s*하면|어떻게\s*할|어쩌지|좋을까|나을까|고민|망설|모르겠|정해야|결정해야)/;
  ...
  const VENT_RE = /(왜\s*맨날|왜\s*항상|왜\s*이렇게|왜\s*나만|왜\s*안\s*될|미치겠|돌아버|속상|답답|서럽|억울|서글|짜증나|힘들어|힘드네|힘들다|외로|지긋지긋|그냥\s*싫|다\s*싫|때려치|하기\s*싫|개많|너무\s*많|할\s*[일게]\s*많|벅차|벅참|정신없|바빠|바쁘다|산더미|지쳤|지쳐|지친|귀찮|멘붕|버겁|죽겠|집중(이|도|은)?\s*(진짜|정말|너무)?\s*안\s*[되돼된됨])/;

  // 이미 끝낸 일(과거형 종결) — 완료 보고는 할 일이 아님
  // 았/었/였 및 축약 과거 음절(했/갔/왔/봤/줬/냈/샀…) + 종결어미
  const DONE_RE = /(했|갔|왔|봤|줬|췄|셨|냈|뺐|썼|났|샀|탔|찼|깼|졌|렸|됐|았|었|였)(다|어|음|네|지|는데|구나|군|어요|다고|단다)|다녀왔|다녀옴|갔다\s*왔|하고\s*왔|돌아왔|끝났|끝냈|마쳤|마무리했/;
  // 아직 안 끝난 의무·계획·바람 — 있으면 완료로 보지 않고 할 일로 남김
  const PENDING_RE = /(해야|하야|가야|봐야|사야|와야|줘야|받아야|보내야|할\s|할걸|할게|할까|할\s*거|할거|하자|하려|하기로|할\s*예정|예정|하고\s*싶|해주|해줘|하는\s*중|해야지|해야겠)/;

  /** 덤프 한 줄의 종류: 'feeling'(감정) | 'ponder'(고민) | 'done'(이미 한 일) | 'task'(할 일) */
  function classifyLine(text) {
    const t = String(text || '').trim();
    if (!t) return 'task';
    if (PONDER_RE.test(t)) return 'ponder';
    const tags = extract(t);
    // 감정 태그(감정/소진성 에너지) 보유 여부 — 완료·짧은 문장 판정에 공용
    const hasMood = tags.some(
      (tg) => tg.type === 'emotion' || (tg.type === 'energy' && FEELING_LABELS.has(tg.label)),
    );
    // 이미 끝낸 일(과거형, 남은 의무·계획 없음)은 할 일이 아님 — 감정 있으면 마음, 없으면 제외
    if (DONE_RE.test(t) && !PENDING_RE.test(t)) return hasMood ? 'feeling' : 'done';
  ...
  // ── 육하원칙(5W1H) 규칙 추출 ──────────────────────────────
  // 누구와(who)·어디서(where)·어떻게(how)·왜(why)를 조사/전치사 패턴으로 뽑는다.
  // 언제(when)는 날짜·시간 파싱이, 무엇을(what)은 제목이 담당.

  // 사람을 가리키는 것이 거의 확실한 단어 — 동반 조사(랑/와/과/하고)와 붙으면 who로 본다
  const WHO_PERSON_HINT = /(엄마|아빠|어머니|아버지|부모님|언니|누나|오빠|형|동생|이모|고모|삼촌|할머니|할아버지|남편|아내|와이프|남친|여친|친구|팀장님?|과장님?|부장님?|대표님?|매니저|선생님|교수님|고객|클라이언트|가족|아이|애들)/;
  // 수단·방법으로 쓰이는 명사 — "(으)로"와 붙으면 how
  const HOW_MEANS_RE = /(전화|이메일|메일|문자|카톡|카카오톡|줌|zoom|구글\s*밋|온라인|오프라인|화상|영상통화|지하철|버스|택시|기차|비행기|자전거|도보|자차|현금|카드|계좌이체)(?:으로|로)(?:\s|$)/i;

  /** 사용자 사전에서 type이 person인 항목이 동반 조사와 함께 등장하면 그 라벨 */
  function whoFromDictionary(text) {
    for (const e of userDictionary) {
      if (e?.type !== 'person' || !e.term) continue;
      const terms = [e.term, ...(e.aliases || [])];
      for (const term of terms) {
        if (term && new RegExp(`${term}(이랑|랑|와|과|하고)`).test(text)) {
          return e.label || e.term;
        }
      }
    }
    return null;
  ...
  /** 육하원칙(5W1H) 규칙 추출 — { who, where, how, why }, 못 찾은 항목은 null */
  function extractFiveW1H(text) {
    const t = String(text || '').trim();
    const out = { who: null, where: null, how: null, why: null };
    if (!t) return out;

    // who — 누구와: ① 사전의 person ② "X랑/와/과/하고 같이·함께" ③ "X랑/와/과 … 만나·약속…" ④ 사람 힌트 단어+조사 ⑤ with X
    let m;
    out.who = whoFromDictionary(t);
    if (!out.who && (m = t.match(/([가-힣]{1,6}?)(?:이랑|랑|와|과|하고)\s*(?:같이|함께)/))) out.who = m[1];
    if (!out.who && (m = t.match(/([가-힣]{1,6}?)(?:이랑|랑|와|과)\s[^,.\n]{0,14}?(?:만나|만남|미팅|회의|약속|점심|저녁|커피|통화|데이트|식사)/))) out.who = m[1];
    if (!out.who && (m = t.match(WHO_PERSON_HINT)) && new RegExp(`${m[1]}(이랑|랑|와|과|하고)`).test(t)) out.who = m[1];
    if (!out.who && (m = t.match(/\bwith\s+(?:my\s+|the\s+)?([A-Za-z][A-Za-z.]{1,15})/i))) out.who = m[1];

    // where — 어디서: "X에서" (처소격, 최대 두 단어) / "at X". 앞의 시간·불용어 토막은 버린다.
    if ((m = t.match(/((?:[가-힣A-Za-z0-9]{1,12}\s+)?[가-힣A-Za-z0-9]{1,12})에서(?:\s|$)/))) {
      const tokens = m[1].trim().split(/\s+/).filter((tk) => !CAND_STOPWORDS.has(tk));
      out.where = tokens.join(' ') || null;
    } else if ((m = t.match(/\bat\s+(?:the\s+)?([A-Za-z][\w-]{1,15})/i))) out.where = m[1];

    // how — 어떻게: 수단 명사+"(으)로" / "걸어서·직접" / "by·via·over X"
    if ((m = t.match(HOW_MEANS_RE))) out.how = m[1];
    else if (/걸어서/.test(t)) out.how = '걸어서';
    else if ((m = t.match(/\b(?:by|via|over)\s+(?:the\s+)?(phone|email|text|zoom|call|bus|train|car|taxi|subway|bike|foot)/i))) out.how = m[1];

    // why — 왜: "X 때문에·하느라" / "X(하)려고·하러" / "X(하)기 위해" / "because X"
    if ((m = t.match(/([가-힣\s]{2,20}?)\s*(때문에|하느라)/))) out.why = `${m[1].trim()} ${m[2]}`;
    else if ((m = t.match(/([가-힣][가-힣\s]{0,18}?(?:려고|하러))(?:\s|$)/))) out.why = m[1].trim();
    else if ((m = t.match(/([가-힣\s]{2,20}?[하기]?)\s*위해(?:서)?/))) out.why = `${m[1].trim()} 위해`;
    else if ((m = t.match(/\bbecause\s+(?:of\s+)?(.{2,30}?)(?:[,.]|$)/i))) out.why = m[1].trim();

    // 각 값 정리 — 너무 길면 자르고, 빈 값은 null
    for (const k of Object.keys(out)) {
      const v = (out[k] || '').toString().trim().slice(0, 24);
      out[k] = v || null;
    }
    return out;
  }
  ...
  /** 로컬 룰만으로 충분한 단순 덤프인지 — true면 AI 호출 없이 처리해 크레딧/토큰을 아낌 */
  function isSimpleDump(text) {
    const lines = parseToMemos(text);
    if (lines.length === 0 || lines.length > 2) return false;
    // 한 줄에 여러 의도가 섞인 신호가 있으면 AI가 쪼개는 게 나음
    const MULTI_INTENT = /(하고\s|그리고|면서|하며|한\s*다음|\band\b|&|,|;)/i;
    return lines.every(
      (line) => line.replace(/\s/g, '').length <= 30 && !MULTI_INTENT.test(line),
    );
  }
