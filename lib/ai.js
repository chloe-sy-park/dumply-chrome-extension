...
Output example: [{"type":"emotion","icon":"😰","label":"stressed"},{"type":"energy","icon":"🥱","label":"sleep-deprived"}]`,
  ...
  const DUMP_PROMPT = {
    ko: (refDate) =>
      '아래 브레인덤프를 의도 단위로 쪼개고 분류해줘. 한 문장에 여러 의도가 섞여 있으면 나눠.\n' +
      'kind 분류 기준:\n' +
      '- "event": 특정 시각에 내가 *가거나 참석*하는 약속·방문·미팅·모임·수업·예약. ("이따 7시에 기린굴 방문" → event, "3시 미팅" → event)\n' +
      '- "task": 내가 *하는* 행동(주문/제출/구매/정하기/준비). 시간이 있어도 행동이면 task. ("7시에 호이 사료 주문" → task, "워리스톤 크기 정하고 가야겠다" → task)\n' +
      '- "remember": 지금 할 행동은 아니지만 *기억·확인·팔로업*할 것. 남이 언급한 약속, 확정 필요, 공유 예정, 결과 대기 등.\n' +
      '- "feeling": 감정·기분·몸상태·푸념. ("배고프다", "피곤해")\n' +
      '- "ponder": 아직 *할지 말지 / 뭘 할지* 못 정한 고민만. ("이직할까 말까") — 막연한 망설임만 ponder.\n' +
      '- title: 군더더기·시간표현 뺀 핵심만 ("이따 7시에 기린굴 방문할건데" → "기린굴 방문")\n' +
      '- time: "HH:MM" 24시간제 또는 null (저녁/밤=오후로 환산)\n' +
      `- date: "오늘"/"내일"/"YYYY-MM-DD" 또는 null (기준일 ${refDate})\n` +
      '- relatedTo: 이 항목이 *다른 항목보다 먼저* 해야 하는 선행이면 *뒤에 오는 항목*의 title, 아니면 null.\n' +
      '  예) "기린굴 방문 전에 워리스톤 정하기" → 워리스톤의 relatedTo="기린굴 방문".\n' +
      'JSON 배열만 응답: [{"title":"기린굴 방문","kind":"event","time":"19:00","date":"오늘","relatedTo":null},{"title":"워리스톤 정하기","kind":"task","time":null,"date":null,"relatedTo":"기린굴 방문"}]\n\n',
