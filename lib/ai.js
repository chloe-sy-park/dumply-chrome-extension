...
const AlfredoAI = (() => {
  ...
  function activeProvider(settings) {
    ...
    // BYO 키가 없으면 Dumply 계정 크레딧으로 (로그인 시)
    if (typeof DumplyAccount !== 'undefined' && DumplyAccount.isSignedIn()) return 'dumply';
    return null;
  }

  async function chat(settings, prompt, maxTokens = 600) {
    const provider = activeProvider(settings) || 'anthropic';
    if (provider === 'dumply') return DumplyAccount.chat(prompt, maxTokens);
...
Output example: [{"type":"emotion","icon":"😰","label":"stressed"},{"type":"energy","icon":"🥱","label":"sleep-deprived"}]`,
  ...
  const DUMP_PROMPT = {
    ko: (refDate) =>
      ...
      '\n## 충실성 (가장 중요)\n' +
      '- title은 사용자가 실제로 쓴 표현을 최대한 *그대로* 써. 시제를 바꾸거나(현재↔과거) 말투를 바꾸거나 문장을 새로 짓지 마.\n' +
      '- 본문에 *없는* 이름·고유명사·숫자·내용을 절대 지어내지 마. ("강아지"를 특정 이름으로 바꾸지 말고 "강아지" 그대로.)\n' +
      '- 아래 예시는 *형식 참고용*일 뿐이야. 예시에 나온 단어(버디, 인디자인, 호이, 기린굴 등)를 출력에 가져다 쓰지 마.\n' +
      '\n## 먼저 줄임말·구어 시간표현 정규화\n' +
      ...
      '\n## 이미 끝낸 일은 할 일이 아님\n' +
      '과거형으로 *이미 한 행동*을 보고하는 문장("산책 다녀왔어", "목욕탕 갔다왔음", "기분전환했다", "카페에서 작업 중이다")은 새로 할 일이 아니야.\n' +
      '- 감정·상태가 함께 묻어나면 "feeling", 아니면 "done"으로 분류해. ("done" 항목은 목록에서 제외돼.)\n' +
      '- 반대로 "운동 가야해", "~해야 한다", "~할 거야"처럼 *아직 안 한 의무·계획*은 task/event로 남겨.\n' +
      '\n## kind 분류 기준\n' +
      '- "event": 특정 시각에 내가 *가거나 참석/방문*하는 약속·미팅·모임·수업. 예약해서 *데려가거나 방문*하는 것도 event. ("내일 오후 1시 미용 예약해서 데려가기" → event, time 13:00, date 내일 / "3시 미팅" → event)\n' +
      '- "task": 내가 *아직 할* 행동(만들기/디자인/작성/주문/제출/구매/정하기/준비/연락/정리). 시간이 있어도 행동이면 task.\n' +
      '- "remember": 지금 할 행동은 아니지만 *기억·확인·팔로업*할 것. 남이 언급한 약속, 답장·결과 대기, 확정 필요, 공유 예정, 날씨·소식 같은 참고. ("업워크 답장 대기" → remember, "내일 비 온다" → remember)\n' +
      '- "feeling": 감정·기분·몸상태·푸념인데 *안에 할 행동이 전혀 없을 때만*. ("배가 아파서 걱정이다", "지금 집중 진짜 안된다", "피곤해")\n' +
      '- "done": 이미 끝낸 행동을 보고만 하는 문장(감정 없음). ("목욕탕 갔다왔음", "산책 다녀옴")\n' +
      '- "ponder": 아직 *할지 말지 / 뭘 할지 / 어떻게 할지* 못 정한 고민. ("이직할까 말까", "어떻게 하면 좋을까") — 망설임·고민만 ponder.\n' +
      '- title: 군더더기·푸념·시간표현만 빼고 사용자의 표현을 살려. 새 문장으로 바꾸지 마.\n' +
      ...
      '\n## 육하원칙(5W1H) 분류\n' +
      '각 항목마다 본문에 *명시된 것만* 추출해 (지어내기 절대 금지, 없으면 null):\n' +
      '- who: 누구와 — 함께하는 사람·대상. ("지수랑 저녁" → "지수", "팀장님과 미팅" → "팀장님")\n' +
      '- where: 어디서 — 장소. ("강남 카페에서" → "강남 카페", "집에서" → "집")\n' +
      '- how: 어떻게 — 수단·방법. ("전화로 문의" → "전화", "줌으로" → "줌")\n' +
      '- why: 왜 — 이유·목적. ("마감이라 서둘러야" → "마감이라서", "선물하려고" → "선물하려고")\n' +
      '언제(when)는 date/time 필드가, 무엇을(what)은 title이 담당한다. 추측하지 말고 본문의 표현을 그대로 써.\n' +
      '\n## 분리 예시 1 (한 줄에 여러 의도 → 각각 분리, 단어는 입력 그대로)\n' +
      '입력: "샘플 디자인 해야하고 내일은 오후 1시에 미용 예약해서 데려가야해. 저녁엔 친구랑 강남에서 약속. 내일 비도 온다네. 와 지금 집중 진짜 안된다. 답장은 왜 아직도 안와 쉬익쉬익"\n' +
      '출력: [' +
      '{"title":"샘플 디자인","kind":"task","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"미용 예약 데려가기","kind":"event","time":"13:00","date":"내일","relatedTo":null,"who":null,"where":null,"how":null,"why":"미용 예약"},' +
      '{"title":"친구랑 약속","kind":"event","time":"18:00","date":"내일","relatedTo":null,"who":"친구","where":"강남","how":null,"why":null},' +
      '{"title":"비 예보 확인","kind":"remember","time":null,"date":"내일","relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"집중 안됨","kind":"feeling","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"답장 대기","kind":"remember","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null}]\n' +
      '\n## 분리 예시 2 (회고형 — 이미 한 일은 done/feeling, 아직 할 일만 task, 고민은 ponder)\n' +
      '입력: "오늘 배가 아파서 걱정이다. 강아지 산책을 하고 목욕탕을 다녀와서 기분전환했다. 카페에서 집필 중이다. 운동을 가야하는데 19:00전에는 가야한다. 어떻게 하면 좋을까?"\n' +
      '출력: [' +
      '{"title":"배가 아파서 걱정","kind":"feeling","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":"배가 아파서"},' +
      '{"title":"강아지 산책, 목욕탕 다녀옴","kind":"done","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"카페에서 집필 중","kind":"done","time":null,"date":null,"relatedTo":null,"who":null,"where":"카페","how":null,"why":null},' +
      '{"title":"운동 가기","kind":"task","time":"19:00","date":"오늘","relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"어떻게 하면 좋을까","kind":"ponder","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null}]\n' +
      'JSON 배열만 응답.\n\n',
    ...
    en: (refDate) =>
      ...
      '- Keep the user\'s own words in "title". Do NOT change tense, reword, or write a new sentence.\n' +
      '- NEVER invent names, proper nouns, numbers, or details that are not in the text (keep "the dog" as "the dog").\n' +
      '- The examples below are for *format only* — do not copy their words (Jake, client, etc.) into your output.\n' +
      ...
      '\n## Already-done actions are NOT tasks\n' +
      'Sentences that *report something already done* in the past tense ("walked the dog", "got back from the bathhouse", "already writing at the cafe") are not new to-dos.\n' +
      '- If emotion/state is attached → "feeling"; otherwise → "done" ("done" items are dropped from the list).\n' +
      '- In contrast, unfinished obligations/plans ("need to work out", "have to ship it", "will call") stay task/event.\n' +
      '\n## kind rules\n' +
      '- "event": appointments I will *attend or go to* at a specific time. ("meeting at 3pm" → event, "dentist at 10" → event, "standup at 9" → event)\n' +
      '- "task": actions I still need to *do*. Includes informal action verbs: ping/message, hit up/contact, circle back/follow up, loop in, ship/deploy/merge, drop/release, reply, set up, send. Even if a time is mentioned, it\'s a task if it\'s an action. ("ping Alex re proposal by EOD" → task, "ship PR before COB" → task)\n' +
      '- "remember": not an immediate action, but something to *track/follow up on*. Pending confirmations, awaited results, things others mentioned.\n' +
      '- "feeling": emotions, mood, physical state, venting — ONLY when no action is embedded. ("burnt out rn" → feeling, "stomach hurts, worried" → feeling)\n' +
      '- "done": a sentence that only reports a finished action with no emotion. ("got back from the bathhouse", "walked the dog already")\n' +
      '- "ponder": undecided dilemmas only. ("idk if I should take the job offer" → ponder, "what should I do?" → ponder)\n' +
      '- title: keep the user\'s wording; strip only filler, venting prefixes, and time expressions. Expand abbreviations. ("mtg w/ client tmrw morning" → "meeting with client")\n' +
      ...
      '\n## 5W1H classification\n' +
      'For each item, extract ONLY what is *explicitly stated* in the text (never invent, use null when absent):\n' +
      '- who: with whom — companion or counterpart. ("dinner with Sam" → "Sam", "1:1 with my manager" → "my manager")\n' +
      '- where: where — place. ("at the Gangnam cafe" → "Gangnam cafe", "at home" → "home")\n' +
      '- how: how — means or method. ("call the clinic" → "phone", "over zoom" → "zoom")\n' +
      '- why: why — reason or purpose. ("since the deadline is close" → "deadline is close", "to buy a gift" → "to buy a gift")\n' +
      'When is covered by date/time; what is covered by title. Keep the user\'s own wording.\n' +
      '\n## Example (reflective dump — done/feeling stay out of the to-do list)\n' +
      'Input: "my stomach hurts today, kinda worried. walked the dog and went to the bathhouse to refresh. writing at the cafe now. need to work out but have to go before 19:00. what should I do?"\n' +
      'Output: [' +
      '{"title":"stomach hurts, worried","kind":"feeling","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":"stomach hurts"},' +
      '{"title":"walked the dog, went to the bathhouse","kind":"done","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"writing at the cafe","kind":"done","time":null,"date":null,"relatedTo":null,"who":null,"where":"cafe","how":null,"why":null},' +
      '{"title":"work out","kind":"task","time":"19:00","date":"today","relatedTo":null,"who":null,"where":null,"how":null,"why":null},' +
      '{"title":"what should I do","kind":"ponder","time":null,"date":null,"relatedTo":null,"who":null,"where":null,"how":null,"why":null}]\n' +
      'JSON array only.\n\n',
  };
  ...
  async function extractDump(settings, text, refDate) {
    ...
    if (!match) return [];
    const parsed = JSON.parse(match[0]);
    const kinds = new Set(['event', 'task', 'remember', 'feeling', 'ponder', 'done']);
    // 5W1H 문자열 정제 — 모델이 "null"을 문자열로 줄 때도 걸러낸다
    const w5 = (v) => {
      const s = v == null ? '' : String(v).trim();
      return s && s.toLowerCase() !== 'null' ? s.slice(0, 40) : null;
    };
    return parsed
      .map((r) => ({
