...
const AlfredoStorage = (() => {
  ...
  const DEFAULT_STATE = {
    ...
    settings: {
      userName: '',
      theme: 'system',           // 'system'(기본: 라이트→mono/다크→mono dark) | light | dark | craft | ash | barbie | midnight | dawn | pale
      accent: '',                // ''(모노) | blue | indigo | green | amber | rose — data-accent 직교 축
      ...
      email: {
        connected: false,
        address: '',
        seenIds: [],       // 이미 Inbox로 가져온 Gmail 메시지 id (중복 방지)
        lastPull: 0,
      },
    },
    top3Expanded: false,
